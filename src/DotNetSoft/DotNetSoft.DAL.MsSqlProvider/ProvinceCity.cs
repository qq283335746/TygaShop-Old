...
namespace DotNetSoft.DAL.MsSqlProvider
{
    public class ProvinceCity : IProvinceCity
    {
        ...
        /// </summary>
        /// <returns></returns>
        public List<Model.ProvinceCity> GetList()
        {
            List<Model.ProvinceCity> list = null;
            ...
            using (SqlDataReader reader = SqlHelper.ExecuteReader(SqlHelper.SqlProviderConnString, CommandType.StoredProcedure, "Pro_GetProvinceCity"))
            {
                if (reader.HasRows)
                {
                    list = new List<Model.ProvinceCity>();
                    while (reader.Read())
                    {
                        Model.ProvinceCity model = new Model.ProvinceCity();
