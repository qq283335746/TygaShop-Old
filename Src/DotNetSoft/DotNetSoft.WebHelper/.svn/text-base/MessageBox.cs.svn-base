...
namespace DotNetSoft.WebHelper
{
    public class MessageBox
    {
        ...
        public static void Messager(Page page, Control control, string msg, string title)
        {
            msg = msg.Replace(@"'", @"“");
            ClientScriptManager csm = page.ClientScript;
            csm.RegisterClientScriptBlock(control.GetType(), control.ClientID, string.Format(@"$.messager.alert('{1}','{0}');", msg, title), true);
        }
        ...
        public static void Show(Page page, Control control, string msg, string url)
        {
            msg = msg.Replace(@"'", @"“");
            ClientScriptManager csm = page.ClientScript;
            csm.RegisterClientScriptBlock(control.GetType(), control.ClientID, string.Format(@"$.messager.alert('{1}','{0}');window.location = {2}", msg, "温馨提醒",url), true);
        }
    }
}
