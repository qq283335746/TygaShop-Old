...
namespace DotNetSoft.DAL.XmlProvider
{
    public class HandlerUserProfile : IHandlerUserProfile
    {
        ...
        public List<Model.HandlerUserProfile> GetList(string userFile)
        {
            List<Model.HandlerUserProfile> list = null;
            ...
            if (q.Count() > 0)
            {
                list = new List<Model.HandlerUserProfile>();
                foreach (var item in q)
                {
                    Model.HandlerUserProfile model = new Model.HandlerUserProfile();
