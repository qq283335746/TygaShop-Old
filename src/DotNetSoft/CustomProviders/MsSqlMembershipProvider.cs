    ...
    public class MsSqlMembershipProvider : MembershipProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (string.IsNullOrEmpty(config["description"]))
            {
                ...
                config.Add("description", SecurityMessage.GetString(SecurityMessage.MembershipSqlProvider_description));
            }
            ...
            _RequiresUniqueEmail = SecurityUtility.GetBooleanValue(config, "requiresUniqueEmail", true);
            ...
            _MinRequiredPasswordLength = SecurityUtility.GetIntValue(config, "minRequiredPasswordLength", 7, false, 128);
            _MinRequiredNonalphanumericCharacters = SecurityUtility.GetIntValue(config, "minRequiredNonalphanumericCharacters", 1, true, 128);
            ...
            if (_MinRequiredNonalphanumericCharacters > _MinRequiredPasswordLength)
                throw new HttpException(SecurityMessage.GetString(SecurityMessage.MinRequiredNonalphanumericCharacters_can_not_be_more_than_MinRequiredPasswordLength));
            ...
            switch (strTemp)
            {
                ...
                case "Hashed":
                    ...
                    break;
                default:
                    throw new ProviderException(SecurityMessage.GetString(SecurityMessage.Provider_bad_password_format));
            }

            if (PasswordFormat == MembershipPasswordFormat.Hashed && EnablePasswordRetrieval)
                throw new ProviderException(SecurityMessage.GetString(SecurityMessage.Provider_can_not_retrieve_hashed_password));
        ...
        {
            SecurityUtility.CheckParameter( ref username, true, true, true, 256, "username" );
            SecurityUtility.CheckParameter( ref oldPassword, true, true, false, 128, "oldPassword" );
            SecurityUtility.CheckParameter( ref newPassword, true, true, false, 128, "newPassword" );
            ...
            {
                throw new ArgumentException(SecurityMessage.GetString(
                              SecurityMessage.Password_too_short,
                              "newPassword",
            ...
            {
                throw new ArgumentException(SecurityMessage.GetString(
                              SecurityMessage.Password_need_more_non_alpha_numeric_chars,
                              "newPassword",
            ...
            if (PasswordStrengthRegularExpression.Length > 0)
            {
                ...
                {
                    throw new ArgumentException(SecurityMessage.GetString(SecurityMessage.Password_does_not_match_regular_expression,
                                                             "newPassword"));
                }
            }
            ...
            if (pass.Length > 128)
            {
                throw new ArgumentException(SecurityMessage.GetString(SecurityMessage.Membership_password_too_long), "newPassword");
            }
            ...
            if (e.Cancel)
            {
                ...
                {
                    throw new ArgumentException(SecurityMessage.GetString(SecurityMessage.Membership_Custom_Password_Validation_Failure), "newPassword");
                }
            }
            ...
            SqlHelper.ExecuteNonQuery(SqlHelper.SqlProviderConnString, CommandType.StoredProcedure, "dbo.Aspnet_Membership_SetPassword", parms);
        ...
        {
            if (!SecurityUtility.ValidateParameter(ref password, true, true, false, 128))
            {
            ...
            if (!SecurityUtility.ValidateParameter(ref encodedPasswordAnswer, RequiresQuestionAndAnswer, true, false, 128))
            {
                status = MembershipCreateStatus.InvalidAnswer;
                return null;
            }

            if (!SecurityUtility.ValidateParameter(ref username, true, true, true, 256))
            {
                status = MembershipCreateStatus.InvalidUserName;
                return null;
            }

            if (!SecurityUtility.ValidateParameter(ref email,
                                               RequiresUniqueEmail,
                                               RequiresUniqueEmail,
                                               false,
                                               256))
            ...
            if (!SecurityUtility.ValidateParameter(ref passwordQuestion, RequiresQuestionAndAnswer, true, false, 256))
            {
        ...
        public override bool DeleteUser(string username, bool deleteAllRelatedData)
        {
            ...
            SqlHelper.ExecuteNonQuery(SqlHelper.SqlProviderConnString, CommandType.StoredProcedure, cmdText, parms.ToArray());
        ...
        public override int GetNumberOfUsersOnline()
        {
            ...
            parms[0].Direction = ParameterDirection.ReturnValue;
            SqlHelper.ExecuteNonQuery(SqlHelper.SqlProviderConnString, CommandType.StoredProcedure, "dbo.Aspnet_Membership_GetNumberOfUsersOnline", parms);
        ...
        public override MembershipUser GetUser(string username, bool userIsOnline)
        {
            ...
            SecurityUtility.CheckParameter(
                            ref username,
                            true,
                            false,
                            true,
        ...
        public override MembershipUser GetUser(object providerUserKey, bool userIsOnline)
        {
            ...
            if (!(providerUserKey is Guid))
            {
                throw new ArgumentException(SecurityMessage.GetString(SecurityMessage.Membership_InvalidProviderUserKey), "providerUserKey");
            }
        ...
        public override void UpdateUser(MembershipUser user)
        {
            ...
            SecurityUtility.CheckParameter(ref temp, true, true, true, 256, "UserName");
            temp = user.Email;
            SecurityUtility.CheckParameter(ref temp,
                                       RequiresUniqueEmail,
                                       RequiresUniqueEmail,
                                       false,
                                       256,
        ...
        {
            if (SecurityUtility.ValidateParameter(ref username, true, true, true, 256) &&
                    SecurityUtility.ValidateParameter(ref password, true, true, false, 128) &&
        ...
        {
            //获取数据集总数
            string cmdText = "select count(*) from [View_Aspnet_MembershipUsers] t1 ";
            ...
            cmdText = @"select * from(select row_number() over(order by CreateDate desc) as RowNumber,
                      UserId,UserName,Email,PasswordQuestion,Comment,IsApproved,CreateDate,
                      LastLoginDate,LastActivityDate,LastPasswordChangedDate,IsLockedOut,LastLockoutDate
                      from [View_Aspnet_MembershipUsers] where 1=1 and ApplicationName = '"+ApplicationName+"' ";
            ...
            using (SqlDataReader reader = SqlHelper.ExecuteReader(SqlHelper.SqlProviderConnString, CommandType.Text, cmdText, commandParameters))
            {
                if (reader != null && reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        email = GetNullableString(reader, 3);
                        passwordQuestion = GetNullableString(reader, 4);
                        comment = GetNullableString(reader, 5);
                        isApproved = reader.GetBoolean(6);
                        dtCreate = reader.GetDateTime(7).ToLocalTime();
                        dtLastLogin = reader.GetDateTime(8).ToLocalTime();
                        dtLastActivity = reader.GetDateTime(9).ToLocalTime();
        ...
        internal string UnEncodePassword(string pass, int passwordFormat)
        {
            switch (passwordFormat)
            {
                ...
                case 1: // MembershipPasswordFormat.Hashed:
                    throw new ProviderException(SecurityMessage.GetString(SecurityMessage.Provider_can_not_decode_hashed_password));
                default:
        ...
        private string GetExceptionText(int status)
        {
            ...
            switch (status)
            {
                case 0:
                    return String.Empty;
                case 1:
                    key = SecurityMessage.Membership_UserNotFound;
                    break;
                case 2:
                    key = SecurityMessage.Membership_WrongPassword;
                    break;
                case 3:
                    key = SecurityMessage.Membership_WrongAnswer;
                    break;
                case 4:
                    key = SecurityMessage.Membership_InvalidPassword;
                    break;
                case 5:
                    key = SecurityMessage.Membership_InvalidQuestion;
                    break;
                case 6:
                    key = SecurityMessage.Membership_InvalidAnswer;
                    break;
                case 7:
                    key = SecurityMessage.Membership_InvalidEmail;
                    break;
                case 99:
                    key = SecurityMessage.Membership_AccountLockOut;
                    break;
                default:
                    key = SecurityMessage.Provider_Error;
                    break;
            }
            return SecurityMessage.GetString(key);
        }
