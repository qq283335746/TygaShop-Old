<%@ Page Title="分类列表" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="ListCategory.aspx.cs" Inherits="DotNetSoft.Web.Admin.Category.ListCategory" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">

<script type="text/javascript" src="../../Scripts/CombotreeFun.js"></script>
...
<div id="toolbar">
  <ul>
      <li class="fl">分类名称：<input type="text" runat="server" id="txtCategoryName" maxlength="50" class="txt" /></li>
      <li class="fl ml10">所属分类：
          <input id="cbtCategory" runat="server" class="easyui-combotree" style="width:250px;" />
      </li>
      <li class="fl ml10">
...
<asp:Repeater ID="rpData" runat="server">
    <HeaderTemplate>
        <table id="bindT" class="easyui-datagrid" title="分类列表" data-options="rownumbers:true,toolbar:'#toolbar'" style="height:auto;">
        <thead>
            <tr>
                <th data-options="field:'f0',checkbox:true"></th>
                <th data-options="field:'f1'">分类名称</th>
                <th data-options="field:'f2'">上级分类</th>
                <th data-options="field:'f3'">备注</th>
            </tr>
        </thead>
        <tbody>
    </HeaderTemplate>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#btnNew").click(function () {
            $("#hIsUpdate").val("1");
            window.location = "AddCategory.aspx";
        })
        ...
        $("#abtnDel").click(function () {
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    var cbl = $('#bindT').datagrid("getSelections");
                    if (cbl.length == 0) {
                        $.messager.alert('错误提醒', '请勾选一行或多行数据再进行操作', 'error');
                        return;
                    }
                    var itemsAppend = "";
                    for (var i = 0; i < cbl.length; i++) {
                        itemsAppend += cbl[i].f0 + "|" + cbl[i].f1 + ",";
                    }
        ...
        CombotreeFun.obj = $('[id$=cbtCategory]');
        CombotreeFun.url = "/ScriptServices/AdminService.asmx/GetTreeJsonForCategory";
        CombotreeFun.init();
    })
</script>
