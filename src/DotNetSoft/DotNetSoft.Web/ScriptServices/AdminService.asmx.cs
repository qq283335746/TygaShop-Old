...
namespace DotNetSoft.Web.ScriptServices
{
    ...
    public class AdminService : System.Web.Services.WebService
    {
        ...
        private void GetChildren(Model.ContentType current, List<Model.ContentType> list, ref string jsonAppend)
        {
            List<Model.ContentType> childList = list.FindAll(delegate(Model.ContentType m) { return m.ParentID.Equals(current.NumberID); });
            if (childList != null)
            {
                int n = 0;
