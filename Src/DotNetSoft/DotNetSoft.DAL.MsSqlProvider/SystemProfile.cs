...
namespace DotNetSoft.DAL.MsSqlProvider
{
    public class SystemProfile : ISystemProfile
    {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public int Update(Model.SystemProfile model)
        {
            if (model == null) return -1;
        ...
        /// <param name="numberId"></param>
        /// <returns></returns>
        public Model.SystemProfile GetModel(string numberId)
        {
            Model.SystemProfile model = null;
        ...
        /// <returns></returns>
        public List<Model.SystemProfile> GetModelInTitle(string titleAppend)
        {
            List<Model.SystemProfile> list = null;
            ...
            using (SqlDataReader reader = SqlHelper.ExecuteReader(SqlHelper.SqlProviderConnString, CommandType.Text, cmdText))
            {
                if (reader != null)
                {
                    list = new List<Model.SystemProfile>();
                    while (reader.Read())
                    {
        ...
        /// <returns></returns>
        public IList<Model.SystemProfile> GetList(int pageIndex, int pageSize, out int totalCount, string sqlWhere, params SqlParameter[] commandParameters)
        {
            //获取数据集总数
            ...
            IList<Model.SystemProfile> list = null;
