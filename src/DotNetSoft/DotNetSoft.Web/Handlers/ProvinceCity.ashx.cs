...
namespace DotNetSoft.Web.Handlers
{
    /// <summary>
    /// ProvinceCity 的摘要说明
    /// </summary>
    public class ProvinceCity : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            List<Model.ProvinceCity> list = WebHelper.ProvinceCityDataProxy.GetListProvinceCity();
            if (list == null || list.Count == 0) return;

            //绑定省份
            List<Model.ProvinceCity> provinceItems = list.FindAll(delegate(Model.ProvinceCity pc) { return pc.ParentID == "1"; });
            ...
            if (!string.IsNullOrEmpty(context.Request.QueryString["pcity"]))
            {
                ...
                if (pcityArrLen == 1)
                {
                    Model.ProvinceCity model = list.Find(delegate(Model.ProvinceCity pc) { return pc.RegionName == pcityArr[0]; });
        ...
        private void GetHighUse(string idAppend, List<Model.ProvinceCity> list)
        {
        ...
        /// 获取所有的省份
        /// </summary>
        private void GetProvince(List<Model.ProvinceCity> list)
        {
            string textAppend = string.Empty;
            if (list != null)
            {
                ...
                foreach (Model.ProvinceCity model in list)
                {
                    if (model.ParentID == "1")
                    {
        ...
        private void GetCityByProvince(string provinceId, List<Model.ProvinceCity> list)
        {
            string textAppend = string.Empty;
            if (list != null)
            {
        ...
        /// <param name="list"></param>
        private void GetCountyByCity(string cityId, List<Model.ProvinceCity> list)
        {
            string textAppend = string.Empty;
            if (list != null)
            {
        ...
        /// <param name="list"></param>
        private void GetProvinceCity(string parentId, IList<Model.ProvinceCity> list)
        {
            string textAppend = string.Empty;
            if (list != null)
            {
        ...
        /// </summary>
        /// <param name="list"></param>
        private void ConsoleProvince(List<Model.ProvinceCity> list)
        {
            string textAppend = string.Empty;
            if (list != null)
            {
        ...
        /// </summary>
        /// <param name="list"></param>
        private void ConsoleCity(List<Model.ProvinceCity> list)
        {
            string textAppend = string.Empty;
            if (list != null)
            {
        ...
        /// </summary>
        /// <param name="list"></param>
        private void ConsoleCounty(List<Model.ProvinceCity> list)
        {
            string textAppend = string.Empty;
            if (list != null)
            {
